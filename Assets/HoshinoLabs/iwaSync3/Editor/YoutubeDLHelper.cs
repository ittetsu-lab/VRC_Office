    ...
    internal static class YoutubeDLHelper
    {
        static string ToolPath
        {
            get
            {
                ...
                return Path.Combine(dir, "AppData", "LocalLow", "VRChat", "VRChat", "Tools", "youtube-dl.exe");
            }
        }

        public static bool Exists => File.Exists(ToolPath);

        public delegate void DataReceivedEventHandler(DataReceivedEventArgs e);

        public static Task Execute(string url, string[] args, DataReceivedEventHandler output, DataReceivedEventHandler error,
